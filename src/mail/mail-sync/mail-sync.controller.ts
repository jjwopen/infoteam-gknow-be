...
export class MailSyncController {
  ...
  @ApiOperation({
    summary: '메일 상세 조회',
    description:
      '메일 본문 HTML, 첨부파일 목록을 포함한 상세 정보를 조회합니다.',
  })
  @ApiParam({
    name: 'id',
    description: '메일 ID',
  ...
  async downloadAttachment(@Param('id') id: string, @Res() res: Response) {
    const file = await this.mailSyncService.downloadAttachment(id);

    const encodedFileName = encodeURIComponent(file.fileName ?? 'attachment');
